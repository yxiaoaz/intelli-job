        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    html.Div(id='input-analysis-result', className='mt-2')
                ])
            ])
        ], md=6),
        ...
        dbc.Card([
            ...
            dbc.CardBody([
                dag.AgGrid(
                    id='job-results-grid',
                    columnDefs=[
                        {"headerName": "公司", "field": "company", "filter": True},
                        {"headerName": "职位", "field": "title", "filter": True},
                        {"headerName": "匹配度", "field": "score", "type": "rightAligned", 
                         "valueFormatter": {"function": "d3.format('.0%')(params.value)"}},
                        {"headerName": "地点", "field": "location"},
                        {"headerName": "类型", "field": "job_type"},
                        {"headerName": "详情", "field": "url", "cellRenderer": "markdown"}
                    ],
                    defaultColDef={
                        "sortable": True,
                        "resizable": True,
                ...
                dbc.Button("导出Excel", id="export-btn", color="primary", className="mt-3")
            ])
        ])
    ]))
], fluid=True)
...
@app.callback(
    ...
    Input("job-results-grid", "rowData")
)
